...
public class QuoteProvider {
  ...
  public static class HistoricData {
     @ContentUri(
         ...
         type = "vnd.android.cursor.dir/historic_data"
     )
     public static final Uri CONTENT_URI = buildUri(Path.HISTORIC_DATA);

     @InexactContentUri(
             name = "HISTORIC_DATA_ID",
             path = Path.HISTORIC_DATA + "/*",
             type = "vnd.android.cursor.item/historic_data",
             whereColumn = HistoricDataColumns.SYMBOL,
             pathSegment = 1
     )
     public static Uri withSymbol(String symbol) { return buildUri(Path.HISTORIC_DATA, symbol); }
    }
}
