...
public class StockTaskService extends GcmTaskService{
    ...
    if (urlStringBuilder != null){
      ...
      try{
        ...
        try {
          ...
          ArrayList<ContentProviderOperation> stockSymbols = Utils.quoteJsonToContentVals(getResponse, mContext);
          if (stockSymbols.size() == 0) {
            result = GcmNetworkManager.RESULT_FAILURE;
            Utils.setStockTaskServiceResult(mContext, stockSymbol, false);
          } else {
            Utils.setStockTaskServiceResult(mContext, stockSymbol, true);
          }
  ...
  static public void setStockStatus(Context c, @StockStatus int stockStatus) {
    ...
    editor.putInt(c.getString(R.string.pref_stock_key), stockStatus);
    editor.apply();
  }

  static public void updateWidgets(Context context) {
    // Setting the package ensures that only components in our app will recieve the broadcast
